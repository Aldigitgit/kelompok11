...
export default function CartPage() {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      const { data, error } = await supabase
        .from("cart_items")
        .select(
          "id, quantity, produk:produk_id (id, judul, harga, url_gambar, status)"
        )
  ...
  const handleCheckout = () => {
    ...
    // Kirim cartItems dan total ke halaman pembayaran
    navigate("/checkout", {
      state: {
        cartItems,
        total,
                ...
                {!item.produk ? (
                  ...
                  <>
                    ...
                    <p className="font-semibold text-lg text-red-600">
                      Rp{" "}
                      {(item.quantity * item.produk.harga).toLocaleString(
                        "id-ID"
                      )}
                    </p>
                  </>
